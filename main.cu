...
int main()
{
	...
	static constexpr int N_x = 128;
	const double U_lid = 0.0288675;// Re * (0.5 / Beta - 0.5) / (double(N_x) * 3.0); // Lid velocity
	...
	double* w = nullptr;
	cudaMalloc((void**)&w, Q * sizeof(double));
	cudaMemcpy(w, h_w, Q * sizeof(double), cudaMemcpyHostToDevice);
	...
	int* Ksi = nullptr;
	cudaMalloc((void**)&Ksi, Q * 3 * sizeof(int));
	cudaMemcpy(Ksi, h_Ksi, Q * 3 * sizeof(int), cudaMemcpyHostToDevice);
	...
	double* d_Rho;
	cudaMalloc((void**)&d_Rho, Cell_Count * sizeof(double));

	double* h_Ux = new double[Cell_Count];
	double* d_Ux;
	cudaMalloc((void**)&d_Ux, Cell_Count * sizeof(double));

	double* h_Uy = new double[Cell_Count];
	double* d_Uy;
	cudaMalloc((void**)&d_Uy, Cell_Count * sizeof(double));

	double* h_Uz = new double[Cell_Count];
	double* d_Uz;
	cudaMalloc((void**)&d_Uz, Cell_Count * sizeof(double));

	double* h_viscousity = new double[Cell_Count];
	double* d_viscousity;
	cudaMalloc((void**)&d_viscousity, Cell_Count * sizeof(double));

	double* h_f = new double[Cell_Count * Q];
	double* d_f;
	cudaMalloc((void**)&d_f, Cell_Count * Q * sizeof(double));

	double* h_f_eq = new double[Cell_Count * Q];
	double* d_f_eq;
	cudaMalloc((void**)&d_f_eq, Cell_Count * Q * sizeof(double));

	double* h_f_new = new double[Cell_Count * Q];
	double* d_f_new;
	cudaMalloc((void**)&d_f_new, Cell_Count * Q * sizeof(double));
	...
	if (start_from_new)
	{
		...
		cudaMemcpy(d_Rho, h_Rho, Cell_Count * sizeof(double), cudaMemcpyHostToDevice);
		cudaMemcpy(d_Ux, h_Ux, Cell_Count * sizeof(double), cudaMemcpyHostToDevice);
		cudaMemcpy(d_Uy, h_Uy, Cell_Count * sizeof(double), cudaMemcpyHostToDevice);
		cudaMemcpy(d_Uz, h_Uz, Cell_Count * sizeof(double), cudaMemcpyHostToDevice);

		eqm_d3q27 << <smallBlocks, blockSize >> > (d_f, d_Rho, d_Ux, d_Uy, d_Uz, Cell_Count, w, Ksi);

		cudaDeviceSynchronize();
		cudaMemcpy(d_f_eq, d_f, Cell_Count * Q * sizeof(double), cudaMemcpyDeviceToDevice);
		cudaMemcpy(d_f_new, d_f, Cell_Count * Q * sizeof(double), cudaMemcpyDeviceToDevice);

		cudaMemcpy(h_Rho, d_Rho, Cell_Count * sizeof(double), cudaMemcpyDeviceToHost);
		writeToFile(0, h_Rho, "Density", outputFile, Cell_Count * sizeof(double));

		cudaMemcpy(h_Ux, d_Ux, Cell_Count * sizeof(double), cudaMemcpyDeviceToHost);
		writeToFile(0, h_Ux, "X_Velocity", outputFile, Cell_Count * sizeof(double));

		cudaMemcpy(h_Uy, d_Uy, Cell_Count * sizeof(double), cudaMemcpyDeviceToHost);
		writeToFile(0, h_Uy, "Y_Velocity", outputFile, Cell_Count * sizeof(double));

		cudaMemcpy(h_Uz, d_Uz, Cell_Count * sizeof(double), cudaMemcpyDeviceToHost);
		writeToFile(0, h_Uz, "Z_Velocity", outputFile, Cell_Count * sizeof(double));

		cudaMemcpy(h_f, d_f, Cell_Count * Q * sizeof(double), cudaMemcpyDeviceToHost);
